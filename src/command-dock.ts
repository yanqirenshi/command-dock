...
export class CommandDock extends HTMLElement {
  ...
  private render() {
    ...
    for (const item of this._items) {
      ...
      const { popup, itemEls } = this.buildPopup(item);
      ...
      this.entries.set(item.id, { item, btn, popup, itemEls });
    }
  ...
  private buildPopup(item: DockItem): {
    popup: HTMLElement;
    itemEls: Array<{ el: HTMLElement; def: PopupItem }>;
  } {
